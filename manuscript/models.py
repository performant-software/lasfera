...
class SlugModel(models.Model):
    ...
    def generate_slug(self):
        """Generate a slug for this model based on name and ensure it is unique.
...
class Location(SlugModel):
    ...
    def generate_slug(self):
        """Generate a slug for this entity based on name or fallback and ensure it is unique.
        Adapted from mep-django."""
        if not self.name or not self.name.strip():
            # Fallback options if name is empty
            if self.placename_id:
                slug_src = self.placename_id
            else:
                slug_src = f"toponym-{self.id}"
        else:
            slug_src = self.name
        self.slug = self.dedupe_slug(slugify(unidecode(slug_src)))

    def get_absolute_url(self):
        """Return the URL for this toponym using the slug"""
        return reverse("toponym_detail", kwargs={"toponym_slug": self.slug})
